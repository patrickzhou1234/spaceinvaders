...
function move(event) {
  if (event.keyCode == "39" && character.offsetLeft < window.innerWidth - character.offsetWidth) {
    $("#character").animate({
        left: "+=100vw"
    }, 3000);
  } else if (event.keyCode == "37" && character.offsetLeft > 0) {
    $("#character").animate({
        left: "-=100vw"
    }, 3000);
...
setInterval(function() {
  ...
  if (character.offsetLeft > window.innerWidth - character.offsetWidth) {
    $(character).stop(true);
